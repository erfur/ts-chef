...
describe("Base64 round-trips", () => {
    ...
    test("url-safe alphabet", () => {
        ...
        const dec = new FromBase64().run(enc as string, ["A-Za-z0-9-_", true, false]);
        expect(byteArrToStr(dec as number[])).toBe("hello world!");
    });

    test("decode known: SGVsbG8gV29ybGQ=", () => {
        expect(byteArrToStr(new FromBase64().run("SGVsbG8gV29ybGQ=", fromArgs) as number[])).toBe("Hello World");
    });
});
