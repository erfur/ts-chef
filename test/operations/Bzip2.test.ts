...
describe("Bzip2 Compress/Decompress", () => {
  ...
  test("Compress and Decompress a string", async () => {
    ...
    const compressed = await compressOp.run(
  ...
  test("Compress and Decompress empty input should throw", async () => {
    ...
    await expect(compressOp.run(inputBuffer, [9, 30])).rejects.toThrow(
      "Please provide an input.",
    );
    await expect(decompressOp.run(inputBuffer, [false])).rejects.toThrow(
      "Please provide an input.",
    );
  });
});
