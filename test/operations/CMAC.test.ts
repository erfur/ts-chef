...
describe("CMAC", () => {
  ...
  test("AES-CMAC 64-byte message", () => {
    const message = hexToAB(
      "6bc1bee22e409f96e93d7e117393172aae2d8a571e03ac9c9eb76fac45af8e5130c81c46a35ce411e56b1193073236b4346a9c7413521db2ad5d233e180b7514",
    );
    const result = op.run(message, [KEY128, "AES"]);
  ...
  test("Invalid key length for AES", () => {
    ...
    expect(() => op.run(new ArrayBuffer(0), [invalidKey, "AES"])).toThrow(
      /The key for AES must be either 16, 24, or 32 bytes/,
    );
  });
  ...
  test("Invalid key length for Triple DES", () => {
    const invalidKey = { string: "1234", option: "Hex" };
    expect(() =>
      op.run(new ArrayBuffer(0), [invalidKey, "Triple DES"]),
    ).toThrow(/The key for Triple DES must be 16 or 24 bytes/);
  });
});
