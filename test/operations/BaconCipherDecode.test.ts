...
describe("BaconCipherDecode", () => {
  ...
  test("Decode Complete alphabet (0/1)", () => {
    ...
    expect(
      op.run("0011100100010110101101110", ["Complete", "0/1", false]),
    ).toBe("HELLO");
  });
  ...
  test("Decode Standard alphabet (0/1)", () => {
    ...
    expect(
      op.run("0011100100010100101001101", [
  ...
  test("Decode Complete alphabet (A/B)", () => {
    ...
    expect(
      op.run("AABBB AABAA ABABB ABABB ABBBA", ["Complete", "A/B", false]),
    ).toBe("HELLO");
  });
  ...
  test("Decode Complete alphabet (Case)", () => {
    ...
    expect(
      op.run("aaBBB aaBaa aBaBB aBaBB aBBBa", ["Complete", "Case", false]),
    ).toBe("HELLO");
  });

  test("Decode with Invert Translation", () => {
    // hello (Complete, 0/1) inverted: h=11000, e=11011, l=10100, l=10100, o=10001
    expect(op.run("1100011011101001010010001", ["Complete", "0/1", true])).toBe(
      "HELLO",
    );
  });
  ...
  test("Decode Complete alphabet (A-M/N-Z first letter)", () => {
    ...
    const helloAMNZ =
      "apple apple Nut Nut Nut apple apple Nut apple apple apple Nut apple Nut Nut apple Nut apple Nut Nut apple Nut Nut Nut apple";
    expect(op.run(helloAMNZ, ["Complete", "A-M/N-Z first letter", false])).toBe(
      "HELLO",
    );
  });
