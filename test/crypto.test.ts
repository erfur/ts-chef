...
function aesEncArgs(keyHex: string, ivHex: string, mode = "CBC", inputType = "Raw", outputType = "Hex"): any[] {
    return [
...
function aesDecArgs(keyHex: string, ivHex: string, mode = "CBC", inputType = "Hex", outputType = "Raw"): any[] {
    return [
        { string: keyHex, option: "Hex" },
        { string: ivHex,  option: "Hex" },
        mode, inputType, outputType,
...
describe("AES Encrypt / Decrypt", () => {
    ...
    test("AES-128 CBC round-trip", () => {
        ...
        const dec = new AESDecrypt().run(enc as string, aesDecArgs(KEY128, IV, "CBC"));
    ...
    test("AES-256 CBC round-trip", () => {
        ...
        const dec = new AESDecrypt().run(enc as string, aesDecArgs(KEY256, IV, "CBC"));
        expect((dec as string).trim()).toBe(PLAINTEXT);
    });

    test("AES-128 ECB round-trip", () => {
        const enc = new AESEncrypt().run(PLAINTEXT, aesEncArgs(KEY128, IV, "ECB"));
        const dec = new AESDecrypt().run(enc as string, aesDecArgs(KEY128, IV, "ECB"));
        expect((dec as string).trim()).toBe(PLAINTEXT);
    });

    test("AES-128 CTR round-trip", () => {
        const enc = new AESEncrypt().run(PLAINTEXT, aesEncArgs(KEY128, IV, "CTR"));
        const dec = new AESDecrypt().run(enc as string, aesDecArgs(KEY128, IV, "CTR"));
        expect((dec as string).trim()).toBe(PLAINTEXT);
    });
    ...
    test("AES-128 ECB/NoPadding known vector (NIST)", () => {
        ...
        expect((enc as string).toLowerCase()).toBe("66e94bd4ef8a2c3b884cfa59ca342b2e");
    });
...
describe("Triple DES Encrypt / Decrypt", () => {
    ...
    test("3DES CBC round-trip", () => {
        ...
        const dec = new TripleDESDecrypt().run(enc as any, symDecArgs(KEY, IV, "CBC"));
    ...
    test("3DES ECB round-trip", () => {
        ...
        const dec = new TripleDESDecrypt().run(enc as any, symDecArgs(KEY, IV, "ECB"));
        expect(abToStr(dec as ArrayBuffer).trim()).toBe(PLAINTEXT);
    });
});
...
describe("Blowfish Encrypt / Decrypt", () => {
    ...
    test("Blowfish CBC round-trip", () => {
        ...
        const dec = new BlowfishDecrypt().run(enc as any, symDecArgs(KEY, IV, "CBC"));
        if ((dec as any) instanceof ArrayBuffer) {
            expect(abToStr(dec as any).trim()).toBe(PLAINTEXT);
        } else {
            expect((dec as string).trim()).toBe(PLAINTEXT);
    ...
    test("Blowfish ECB round-trip", () => {
        ...
        const dec = new BlowfishDecrypt().run(enc as any, symDecArgs(KEY, IV, "ECB"));
        if ((dec as any) instanceof ArrayBuffer) {
            expect(abToStr(dec as any).trim()).toBe(PLAINTEXT);
        } else {
            expect((dec as string).trim()).toBe(PLAINTEXT);
