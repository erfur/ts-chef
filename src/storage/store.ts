...
export class VariableStore {
    ...
    save(vars: Variable[]): void {
        const dir = getStoreDir();
        if (!dir) { vscode.window.showWarningMessage("ts-chef: open a workspace folder to save variables."); return; }
        ensureDir(dir);
...
export class PipelineStore {
    ...
    save(pipelines: Pipeline[]): void {
        const dir = getStoreDir();
        if (!dir) { vscode.window.showWarningMessage("ts-chef: open a workspace folder to save pipelines."); return; }
        ensureDir(dir);
