...
export class DecorationProvider {
    ...
    constructor(private state: ScanState) {
        this.enabled = vscode.workspace.getConfiguration("tschef").get("highlightingEnabled", true);
    }

    toggle(): void {
        this.enabled = !this.enabled;
        if (!this.enabled) this.clearAll();
        vscode.workspace.getConfiguration("tschef").update("highlightingEnabled", this.enabled, vscode.ConfigurationTarget.Global);
    }
    ...
    update(editor: vscode.TextEditor): void {
        ...
        for (const m of matches) {
            ...
            const label = m.matches.map(r => `${r.label} (${Math.round(r.confidence * 100)}%)`).join(", ");
            const opts: vscode.DecorationOptions = {
                range: m.range,
                hoverMessage: new vscode.MarkdownString(`**ts-chef detected:** ${label}`),
            };
