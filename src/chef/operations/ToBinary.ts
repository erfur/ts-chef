...
export class ToBinary extends Operation {
    constructor() {
        ...
        this.args = [
            { name: "Delimiter", type: "option", value: ["Space", "Comma", "Semi-colon", "Colon", "Line feed", "CRLF", "None"] },
    ...
    run(input: ArrayBuffer, args: unknown[]): string {
        ...
        return Array.from(bytes).map(b => b.toString(2).padStart(byteLen, "0")).join(delim);
    }
}
