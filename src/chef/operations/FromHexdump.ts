...
export class FromHexdump extends Operation {
    ...
    run(input: string, _args: unknown[]): number[] {
        const output: number[] = [];
        const regex =
            /^\s*(?:[\dA-F]{4,16}h?:?)?[ \t]+((?:[\dA-F]{2} ){1,8}(?:[ \t]|[\dA-F]{2}-)(?:[\dA-F]{2} ){1,8}|(?:[\dA-F]{4} )+(?:[\dA-F]{2})?|(?:[\dA-F]{2} )*[\dA-F]{2})/igm;
    ...
    highlightReverse(pos: HighlightPos, args: unknown[]): HighlightResult {
        ...
        for (let i = 1; i < len; i++) {
            ...
            const start =
                ((pos[i].start - lineNum * width - 10) / 3) + (width - w - 2) + lineNum * width;
            const end =
                ((pos[i].end + 1 - lineNum * width - 10) / 3) + (width - w - 2) + lineNum * width;
            pos.push({ start, end });
        }
