...
export class ToHTMLEntity extends Operation {
    constructor() {
        ...
        this.args = [
            ...
            { name: "Format", type: "option", value: ["Named entities where possible, otherwise decimal", "Named entities where possible, otherwise hex", "Decimal", "Hex"] },
        ];
    }
    ...
    run(input: string, args: unknown[]): string {
        ...
        return Array.from(input).map(ch => {
            ...
            if (!convertAll && cp < 128 && cp !== 38 && cp !== 60 && cp !== 62 && cp !== 34 && cp !== 39) {
                return ch;
            }
