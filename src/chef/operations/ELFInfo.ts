...
export class ELFInfo extends Operation {
  ...
  run(input: ArrayBuffer, args: any[]): string {
    ...
    function elfHeader(stream: Stream): string {
      ...
      if (
        !magic ||
        Array.from(magic).join(",") !== [0x7f, 0x45, 0x4c, 0x46].join(",")
      )
        throw new OperationError("Invalid ELF");

      ehResult.push(
        "Magic:".padEnd(align) + `${Utils.byteArrayToChars(Array.from(magic))}`,
      );

      format = stream.readInt(1) ?? 0;
      ehResult.push(
        "Format:".padEnd(align) + `${format === 1 ? "32-bit" : "64-bit"}`,
      );
      ...
      ehResult.push(
        "Version:".padEnd(align) + `${(stream.readInt(1) ?? 0).toString()}`,
      );
      ...
      ehResult.push(
        "ELF Version:".padEnd(align) + `${stream.readInt(4, endianness) ?? 0}`,
      );
      ...
      ehResult.push(
        "ELF Header Size:".padEnd(align) +
          `${stream.readInt(2, endianness) ?? 0} bytes`,
      );
      ehResult.push(
      ...
      ehResult.push(
        "Program Header Entries:".padEnd(align) + phEntries.toString(),
      );
      ...
      ehResult.push(
        "Section Header Entries:".padEnd(align) + shEntries.toString(),
      );
      shstrtab = stream.readInt(2, endianness) ?? 0;
      ehResult.push(
        "Section Header Names:".padEnd(align) + shstrtab.toString(),
      );
    ...
    function programHeader(stream: Stream): string {
      ...
      if (format === 2)
        phResult.push(
          "Flags:".padEnd(align) +
            readFlags(stream.readInt(4, endianness) ?? 0),
        );

      const readSize = format === 1 ? 4 : 8;
      phResult.push(
        "Offset Of Segment:".padEnd(align) +
          `${stream.readInt(readSize, endianness) ?? 0}`,
      );
      phResult.push(
      ...
      phResult.push(
        "Size of Segment:".padEnd(align) +
          `${stream.readInt(readSize, endianness) ?? 0} bytes`,
      );
      phResult.push(
    ...
    function sectionHeader(stream: Stream): string {
      ...
      shResult.push(
        "Section Vaddr in memory:".padEnd(align) + vaddr.toString(),
      );

      const shoffset = stream.readInt(readSize, endianness) ?? 0;
      shResult.push(
        "Offset of the section:".padEnd(align) + shoffset.toString(),
      );
      ...
      shResult.push(
        "Associated Section:".padEnd(align) + associatedSection.toString(),
      );

      const extraInfo = stream.readInt(4, endianness) ?? 0;
      shResult.push(
        "Section Extra Information:".padEnd(align) + extraInfo.toString(),
      );
    ...
    const result: string[] = [
      "=".repeat(align) + " ELF Header " + "=".repeat(align),
    ];
