...
export class SM4Encrypt extends Operation {
    ...
    run(input: string, args: unknown[]): string {
        ...
        if (key.length !== 16)
            throw new OperationError(`Invalid key length: ${key.length} bytes. SM4 uses 16 bytes.`);
        if (iv.length !== 16 && !mode.startsWith("ECB"))
            throw new OperationError(`Invalid IV length: ${iv.length} bytes. SM4 uses 16 bytes.`);

        const inputBytes = Utils.convertToByteArray(input, inputType);
        const output = encryptSM4(inputBytes, key, iv, mode.substring(0, 3));
