...
type XORScheme = "Standard" | "Input differential" | "Output differential" | "Cascade";
...
export class XOR extends Operation {
    constructor() {
        ...
        this.args = [
            ...
            { name: "Scheme", type: "option", value: ["Standard", "Input differential", "Output differential", "Cascade"] },
    ...
    run(input: ArrayBuffer, args: unknown[]): ArrayBuffer {
        ...
        if (scheme === "Standard") {
            for (let i = 0; i < data.length; i++) {
                if (nullPreserving && (data[i] === 0 || data[i] === keyBytes[i % keyBytes.length])) {
                    out[i] = data[i];
                } else {
            ...
            let prev = 0;
            for (let i = 0; i < data.length; i++) {
                if (nullPreserving && data[i] === 0) { out[i] = data[i]; prev = data[i]; continue; }
            ...
            let prev = 0;
            for (let i = 0; i < data.length; i++) {
                if (nullPreserving && data[i] === 0) { out[i] = data[i]; prev = out[i]; continue; }
                out[i] = (data[i] ^ prev ^ keyBytes[i % keyBytes.length]) & 0xff;
                prev = out[i];
            }
