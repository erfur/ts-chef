...
import { encryptRC6, getBlockSize, getDefaultRounds } from "../lib/RC6";

/**
 * RC6 Encrypt operation
 */
export class RC6Encrypt extends Operation {
  /**
  ...
  run(input: any, args: any[]): any {
    ...
    // Validate word size
    if (
      !Number.isInteger(wordSize) ||
      wordSize < 8 ||
      wordSize > 256 ||
      wordSize % 8 !== 0
    )
      throw new OperationError(
        `Invalid word size: ${wordSize}. Must be a multiple of 8 between 8 and 256.`,
      );
