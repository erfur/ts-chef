...
export class IPv6TransitionAddresses extends Operation {
  ...
  run(input: string, args: any[]): string {
    ...
    const ipTransition = (input: string, range: boolean): string => {
      ...
      output +=
        "2002:" + hexify(HEXIP[0]) + hexify(HEXIP[1]) + ":" + hexify(HEXIP[2]);
      if (range) {
        output += "00::/40\n";
      } else {
    ...
    const macTransition = (input: string): string => {
      ...
      const MAC =
        ...
        MACPARTS[5];
      output +=
        MAC.slice(0, 1) + XOR[MAC.slice(1, 2).toLowerCase()] + MAC.slice(2);
    ...
    const unTransition = (input: string): string => {
      ...
      if (input.startsWith("2002:")) {
        ...
        output +=
          String(intify(input.slice(5, 7))) +
          "." +
          String(intify(input.slice(7, 9))) +
          "." +
          String(intify(input.slice(10, 12))) +
          "." +
          String(intify(input.slice(12, 14))) +
          "\n";
      } else if (
        input.startsWith("::ffff:") ||
        input.startsWith("0000:0000:0000:0000:0000:ffff:") ||
        input.startsWith("::ffff:0000:") ||
        input.startsWith("0000:0000:0000:0000:ffff:0000:") ||
        input.startsWith("64:ff9b::") ||
        input.startsWith("0064:ff9b:0000:0000:0000:0000:")
      ) {
        /**
        ...
        if (match1 && match2) {
          ...
          output +=
            intify(hextets.slice(-8, -7) + hextets.slice(-7, -6)) +
            "." +
            intify(hextets.slice(-6, -5) + hextets.slice(-5, -4)) +
            "." +
            intify(hextets.slice(-4, -3) + hextets.slice(-3, -2)) +
            "." +
            intify(hextets.slice(-2, -1) + hextets.slice(-1)) +
            "\n";
        }
