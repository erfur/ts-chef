...
export class ToTable extends Operation {
    constructor() {
        ...
        this.args = [
            ...
            { name: "Treat multiple delimiters as one", type: "boolean", value: false },
        ];
    }

    run(input: string, args: unknown[]): string {
        const cellDelim = (args[0] as string).replace(/\\n/g, "\n").replace(/\\t/g, "\t") || ",";
        const rowDelim = (args[1] as string).replace(/\\n/g, "\n").replace(/\\t/g, "\t") || "\n";
        ...
        function splitRow(row: string): string[] {
            if (multipleDelims) {
                return row.split(new RegExp(`[${cellDelim.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&")}]+`));
            }
