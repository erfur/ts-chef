...
export class ParseTLSRecord extends Operation {
  ...
  _readRecord(input: Stream): any {
    ...
    const recordHeader = new RecordHeader(
      typeString,
      version,
      length,
      truncated,
...
class HandshakeParser {
  ...
  parse(input: Stream, recordHeader: RecordHeader): any {
    ...
    output.handshakeType =
      this._handshakeTypes[handshakeType] ?? handshakeType.toString();
    ...
    switch (handshakeType) {
      case HandshakeType.CLIENT_HELLO:
        return {
          ...output,
          ...this._clientHelloParser.parse(new Stream(content)),
        };
      case HandshakeType.SERVER_HELLO:
        return {
          ...output,
          ...this._serverHelloParser.parse(new Stream(content)),
        };
      case HandshakeType.NEW_SESSION_TICKET:
        return {
          ...output,
          ...this._newSessionTicketParser.parse(new Stream(content)),
        };
      case HandshakeType.CERTIFICATE:
        return {
          ...output,
          ...this._certificateParser.parse(new Stream(content)),
        };
      case HandshakeType.CERTIFICATE_REQUEST:
        return {
          ...output,
          ...this._certificateRequestParser.parse(new Stream(content)),
        };
      case HandshakeType.CERTIFICATE_VERIFY:
        return {
          ...output,
          ...this._certificateVerifyParser.parse(new Stream(content)),
        };
      default:
...
class ClientHelloParser {
  ...
  _readCipherSuites(input: Stream): any {
    ...
    const cipherSuites = new Stream(
      input.getBytes(output.length) ?? new Uint8Array(),
    );
  ...
  _readCompressionMethods(input: Stream): any {
    ...
    const compressionMethods = new Stream(
      input.getBytes(output.length) ?? new Uint8Array(),
    );
...
class CertificateRequestParser {
  ...
  parse(input: Stream): any {
    ...
    output.supportedSignatureAlgorithms =
      this._readSupportedSignatureAlgorithms(input);
  ...
  _readCertificateTypes(input: Stream): any {
    ...
    const certificateTypes = new Stream(
      input.getBytes(output.length) ?? new Uint8Array(),
    );
  ...
  _readSupportedSignatureAlgorithms(input: Stream): any {
    ...
    const signatureAlgorithms = new Stream(
      input.getBytes(output.length) ?? new Uint8Array(),
    );
  ...
  _readCertificateAuthorities(input: Stream): any {
    ...
    const certificateAuthorities = new Stream(
      input.getBytes(output.length) ?? new Uint8Array(),
    );
    ...
    while (certificateAuthorities.hasMore()) {
      const certificateAuthority = this._readCertificateAuthority(
 ...
 */
function readSizePrefixedBytesAsHex(
  input: Stream,
  sizePrefixLength: number,
): string {
