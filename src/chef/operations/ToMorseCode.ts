...
export class ToMorseCode extends Operation {
    constructor() {
        ...
        this.args = [
            { name: "Format", type: "option", value: ["Dots/Dashes", "Binary", "Decimal", "Hex", "BART"] },
            { name: "Letter delimiter", type: "option", value: ["Space", "Line feed", "CRLF", "Forward slash", "Backslash", "Comma", "Semi-colon"] },
            { name: "Word delimiter", type: "option", value: ["Line feed", "CRLF", "Space", "Forward slash", "Backslash", "Comma", "Semi-colon"] },
        ];
    }
    ...
    run(input: string, args: unknown[]): string {
        ...
        const letterDelimOpts: Record<string, string> = {
            "Space": " ", "Line feed": "\n", "CRLF": "\r\n",
            "Forward slash": "/", "Backslash": "\\", "Comma": ",", "Semi-colon": ";",
        };
        ...
        const encodedWords = words.map(word => {
            const letters = Array.from(word).map(ch => {
                ...
                if (format === "Binary") return code.split("").map(c => c === "." ? "10" : "1110").join("0") + "000";
                if (format === "Decimal") return code.split("").map(c => c === "." ? "1" : "7").join("0") + "0";
                if (format === "Hex") return code.split("").map(c => c === "." ? "1" : "7").join("0") + "0";
                if (format === "BART") return code.replace(/\./g, "dot").replace(/-/g, "dash");
                return code;
            }).filter(s => s !== "");
