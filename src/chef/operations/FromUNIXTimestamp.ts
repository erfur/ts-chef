...
export class FromUNIXTimestamp extends Operation {
    ...
    run(input: number, args: unknown[]): string {
        ...
        if (units === "Seconds (s)") {
            return moment.unix(val).tz("UTC").format("ddd D MMMM YYYY HH:mm:ss") + " UTC";
        } else if (units === "Milliseconds (ms)") {
            return moment(val).tz("UTC").format("ddd D MMMM YYYY HH:mm:ss.SSS") + " UTC";
        } else if (units === "Microseconds (us)") {
            return moment(val / 1000).tz("UTC").format("ddd D MMMM YYYY HH:mm:ss.SSS") + " UTC";
        } else if (units === "Nanoseconds (ns)") {
            return (
                moment(val / 1000000).tz("UTC").format("ddd D MMMM YYYY HH:mm:ss.SSS") + " UTC"
            );
        } else {
