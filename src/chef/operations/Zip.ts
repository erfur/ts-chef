...
export class Zip extends Operation {
    constructor() {
        ...
        this.args = [
            ...
            { name: "Compression level", type: "option", value: ["Default Compression", "No Compression", "Best Speed", "Best Compression"] },
    ...
    async run(input: ArrayBuffer, args: unknown[]): Promise<ArrayBuffer> {
        ...
        try {
            const zip = new JSZip();
            zip.file(filename, new Uint8Array(input), { comment, compression: "DEFLATE", compressionOptions: { level } });
