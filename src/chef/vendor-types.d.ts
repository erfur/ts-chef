...
declare module "jsqr" {
    ...
    export default function jsQR(data: Uint8ClampedArray | Uint8Array, width: number, height: number, options?: { inversionAttempts?: string }): QRCode | null;
}
...
declare module "protobufjs" {
    ...
    export class Field {
        constructor(name: string, id: number, type: string, rule?: string, extend?: string, options?: Record<string, unknown>);
        name: string;
        id: number;
        type: string;
        rule?: string;
    ...
    export function parse(protoSource: string, root?: Root, options?: unknown): ParseResult;
    export function load(filename: string, callback: (err: Error | null, root: Root) => void): void;
...
declare module "jsesc" {
    ...
    function jsesc(input: string | number | Record<string, unknown> | unknown[], options?: JsescOptions): string;
    export = jsesc;
}
...
declare module "zlibjs/bin/gzip.min.js" {
    interface GzipOptions {
        ...
        flags?: { fhcrc?: boolean; fname?: boolean; comment?: boolean; fextra?: boolean };
        filename?: string;
        comment?: string;
    }
    class Gzip {
...
declare module "lz4js" {
    export function compress(src: Uint8Array | Buffer, maxSize?: number): Uint8Array;
    export function decompress(src: Uint8Array | Buffer, maxSize?: number): Uint8Array;
...
declare module "otpauth" {
    ...
    class TOTP {
        ...
        validate(options: { token: string; timestamp?: number; window?: number }): number | null;
    ...
    class HOTP {
        ...
        validate(options: { token: string; counter?: number; window?: number }): number | null;
        readonly issuer: string;
        readonly label: string;
        readonly algorithm: string;
        readonly digits: number;
...
declare module "node-md6" {
    function md6(input: string | Buffer, bits?: number, key?: string, levels?: number): string;
    export = md6;
}
...
declare module "tesseract.js" {
    interface Worker {
        ...
        recognize(image: string | Buffer | HTMLImageElement): Promise<{ data: { text: string; hocr: string; confidence: number } }>;
...
declare module "es6-promisify" {
    function promisify<T = unknown>(fn: (...args: unknown[]) => void): (...args: unknown[]) => Promise<T>;
    export = promisify;
}
...
declare module "@alexaltea/capstone-js/dist/capstone.min.js" {
    ...
    class Capstone {
        ...
        disasm(buffer: Uint8Array | number[], addr: number, max?: number): Instruction[];
        disasm_lite(buffer: Uint8Array | number[], addr: number, max?: number): [number, number, string, string][];
        close(): void;
    }
    const cs: {
        ARCH_ARM: number;
    ...
    export { ARCH_ARM, ARCH_ARM64, ARCH_X86, ARCH_MIPS, ARCH_PPC, ARCH_SPARC, MODE_ARM, MODE_THUMB, MODE_16, MODE_32, MODE_64, MODE_BIG_ENDIAN, MODE_LITTLE_ENDIAN, MODE_MCLASS, MODE_V8, Capstone, cs };
}
...
declare module "escodegen" {
    ...
    export function attachComments(ast: unknown, comments: unknown[], tokens: unknown[]): unknown;
}
...
declare module "terser" {
    ...
    export function minify(code: string | Record<string, string>, options?: MinifyOptions): Promise<MinifyOutput>;
}
