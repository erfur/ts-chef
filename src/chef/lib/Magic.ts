...
class Magic {
    ...
    async detectLanguage(optionArray: boolean[]): Promise<Array<{ lang: string; score: number }>> {
        void optionArray;
        return [];
    }

    async detectFileType(): Promise<{ name: string; mime: string; extension: string } | null> {
        return null;
    }

    isUTF8(): boolean {
        try {
            const buf = typeof this.data === "string"
