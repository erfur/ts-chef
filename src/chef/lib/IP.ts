...
export function ipv4CidrRange(
  cidr: RegExpMatchArray,
  includeNetworkInfo: boolean,
  enumerateAddresses: boolean,
  allowLargeList: boolean,
): string {
...
export function ipv6CidrRange(
  ...
  if (includeNetworkInfo) {
    ...
    output +=
      "Total addresses in range: " + (parseInt(total.join(""), 2) + 1) + "\n\n";
  }
 ...
 */
export function ipv6HyphenatedRange(
  range: RegExpMatchArray | string[],
  includeNetworkInfo: boolean,
): string {
 ...
 * about it.
 */
export function ipv6ListedRange(
  match: RegExpMatchArray,
  includeNetworkInfo: boolean,
): string {
