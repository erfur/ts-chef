 ...
 */
export function ipv4CidrRange(cidr: RegExpMatchArray, includeNetworkInfo: boolean, enumerateAddresses: boolean, allowLargeList: boolean): string {
 ...
 */
export function ipv6CidrRange(cidr: RegExpMatchArray, includeNetworkInfo: boolean): string {
    let output = "";
 ...
 * about it.
 */
export function ipv4HyphenatedRange(range: RegExpMatchArray | string[], includeNetworkInfo: boolean, enumerateAddresses: boolean, allowLargeList: boolean): string {
 ...
 */
export function ipv6HyphenatedRange(range: RegExpMatchArray | string[], includeNetworkInfo: boolean): string {
 ...
 * about it.
 */
export function ipv4ListedRange(match: RegExpMatchArray, includeNetworkInfo: boolean, enumerateAddresses: boolean, allowLargeList: boolean): string {
 ...
 * about it.
 */
export function ipv6ListedRange(match: RegExpMatchArray, includeNetworkInfo: boolean): string {
 ...
 */
export function strToIpv4(ipStr: string): number {
     ...
     */
    function parseBlocks(blocks: string[]): number[] {
        if (blocks.length !== 4)
 ...
 */
export function ipv4ToStr(ipInt: number): string {
 ...
 */
export function strToIpv6(ipStr: string): number[] {
    let j = 0;
    const blocks = ipStr.split(":"),
        numBlocks = parseBlocks(blocks),
        ipv6 = new Array(8);
    ...
    function parseBlocks(blocks: string[]): number[] {
        ...
        for (let i = 0; i < blocks.length; i++) {
            ...
            if (isNaN(numBlocks[i])) {
                numBlocks[i] = NaN; // Keep it NaN for the caller to handle
            } else if (numBlocks[i] < 0 || numBlocks[i] > 65535) {
                throw new OperationError("Block out of range.");
            }
        }
        return numBlocks;
 ...
 */
export function generateIpv4Range(ip: number, endIp: number): string[] {
    const range: string[] = [];
    if (endIp >= ip) {
        for (; ip <= endIp; ip++) {
 ...
 */
export function genIpv6Mask(cidr: number): number[] {
    ...
    for (let i = 0; i < 8; i++) {
        if (cidr > ((i + 1) * 16)) {
            ...
            shift = cidr - (i * 16);
            if (shift < 0) shift = 0;
            mask[i] = ~((0x0000FFFF >>> shift) | 0xFFFF0000) & 0xFFFF;
        }
    }
 ...
 */
export function ipv4Compare(a: string, b: string): number {
 ...
 */
export function ipv6Compare(a: string, b: string): number {
 ...
 */
export const protocolLookup: { [key: number]: ProtocolInfo } = {
