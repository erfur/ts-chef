...
export interface FuzzyMatchWeights {
    sequentialBonus: number;
    separatorBonus: number;
    camelBonus: number;
    firstLetterBonus: number;
    leadingLetterPenalty: number;
    maxLeadingLetterPenalty: number;
    unmatchedLetterPenalty: number;
}

export const DEFAULT_WEIGHTS: FuzzyMatchWeights = {
 ...
 * @param str            string which is being searched
 * @param global         whether to search for all matches or just one
 * @returns a boolean which tells if pattern was found or not and a search score
 */
export function fuzzyMatch(pattern: string, str: string, global: true, weights?: FuzzyMatchWeights): Array<[boolean, number, number[]]>;
export function fuzzyMatch(pattern: string, str: string, global?: false, weights?: FuzzyMatchWeights): [boolean, number, number[]];
export function fuzzyMatch(
    pattern: string,
    str: string,
    global = false,
    weights: FuzzyMatchWeights = DEFAULT_WEIGHTS
): [boolean, number, number[]] | Array<[boolean, number, number[]]> {
    ...
    let foundMatch = true,
        ...
        idxs: number[],
        strCurrIndex = 0;
    const results: Array<[boolean, number, number[]]> = [];
...
function fuzzyMatchRecursive(
    ...
    while (patternCurIndex < pattern.length && strCurrIndex < str.length) {
        ...
        if (
            ...
            const [matched, recursiveScore, recursiveMatchesOut] = fuzzyMatchRecursive(
                pattern,
                str,
                patternCurIndex,
                strCurrIndex + 1,
            ...
            if (matched) {
                ...
                if (!recursiveMatch || recursiveScore > bestRecursiveScore) {
                    bestRecursiveMatches = [...recursiveMatchesOut];
 ...
 * @param matches - array of match indexes
 * @returns array of [start, length] pairs
 */
export function calcMatchRanges(matches: number[]): [number, number][] {
