...
export const RADIX_DELIM_OPTIONS_TO = RADIX_DELIM_OPTIONS.filter(d => d !== "Auto");
...
export function delimChar(name: string): string {
    return Object.prototype.hasOwnProperty.call(DELIM_CHARS, name) ? DELIM_CHARS[name] : name;
}
 ...
 */
export function fromRadix(input: string, delimName: string, radix: number, digitLen: number): number[] {
    if (!input || !input.trim()) return [];
    if (radix < 2 || radix > 36) throw new OperationError("Base must be between 2 and 36");
    ...
    return parts
        ...
        .map((p, idx) => {
            ...
            if (isNaN(n)) {
                throw new OperationError(`Invalid value "${p}" at token ${idx + 1} for base ${radix}`);
            }
