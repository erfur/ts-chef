...
class Protobuf {
     ...
     * @returns {object}
     */
    static decode(input: number[] | Uint8Array, args: any[]): Record<string, unknown> {
        Protobuf.updateProtoRoot(args[0]);
    ...
    static updateProtoRoot(protoText: string): void {
        try {
            ...
            if (Protobuf.parsedProto.package) {
                Protobuf.parsedProto.root = Protobuf.parsedProto.root.nested[Protobuf.parsedProto.package];
            }
    ...
    static updateMainMessageName(): void {
        ...
        Protobuf.parsedProto.root.nestedArray.forEach((block: any) => {
            if (block instanceof protobuf.Type) {
                ...
                Protobuf.parsedProto.root.nested[block.name].fieldsArray.forEach((field: any) => {
    ...
    static mergeDecodes(input: number[] | Uint8Array): Record<string, unknown> {
        ...
        if (Protobuf.showTypes) {
            ...
            Protobuf.parsedProto.root = Protobuf.appendTypesToFieldNames(Protobuf.parsedProto.root);
        }
    ...
    static appendTypesToFieldNames(schemaRoot: any): any {
        for (const block of schemaRoot.nestedArray) {
            if (block instanceof protobuf.Type) {
                for (const [fieldName, fieldData] of Object.entries(block.fields) as [string, any][]) {
     ...
     */
    static showRawTypes(rawDecode: Record<string, unknown>, fieldTypes: Record<string, any>): Record<string, unknown> {
        for (const [fieldNum, value] of Object.entries(rawDecode)) {
            ...
            if (isNaN(fieldType as number)) {
                ...
                if (Array.isArray(value)) {
                    ...
                    for (const instance of Object.keys(value)) {
                        if (typeof (value as any)[instance] !== "string") {
                            fieldInstances.push(Protobuf.showRawTypes((value as any)[instance], fieldType));
                        } else {
                ...
                // Single submessage
                    ...
                    outputFieldValue = Protobuf.showRawTypes(value as Record<string, unknown>, fieldType);
                }
            ...
            rawDecode[`field #${fieldNum}: ${Protobuf.getTypeInfo(outputFieldType as number)}`] = outputFieldValue;
     ...
     * @returns {Record<string, unknown>}
     */
    static compareFields(rawDecodedMessage: Record<string, unknown>, schemaMessage: any): Record<string, unknown> {
        ...
        for (const fieldName in rawDecodedMessage) {
            ...
            if (fieldId in schemaFieldProperties) {
                ...
                if (schemaField.resolvedType instanceof protobuf.Type) {
                    ...
                    const schemaSubMessage = Protobuf.parsedProto.root.nested[subMessageType];
     ...
     * @returns {Record<string, unknown>}
     */
    _addField(field: { key: number | string; value: unknown }, object: Record<string, unknown>): Record<string, unknown> {
    ...
    _lenDelim(fieldNum: number | string): Record<string, unknown> | string {
        ...
        const fieldBytes = this.data.slice(this.offset, this.offset + length) as number[] | Uint8Array;
    ...
    _uint32(): number {
        ...
        const dataview = new DataView(new Uint8Array(Array.from(this.data.slice(this.offset, this.offset + 4))).buffer);
