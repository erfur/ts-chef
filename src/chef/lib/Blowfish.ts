...
export class BlowfishAlgorithm {
    ...
    constructor(key: string | number[], modeName: string) {
        ...
        const modeOption = {
            blockSize: 8,
            cipher: {
                encrypt: function (inblock: number[], outblock: number[]) {
                    encryptBlock(inblock, outblock, _box);
                },
                decrypt: function (inblock: number[], outblock: number[]) {
    ...
    initialize(options: { key: string | number[] }) {
        ...
        if (typeof options.key === "string") {
            k = options.key.split("").map((item) => item.charCodeAt(0) & 0xff);
        } else {
            k = options.key;
        }
