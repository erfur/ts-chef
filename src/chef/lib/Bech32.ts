...
const CHARSET_REV: { [key: string]: number } = {};
 ...
 * @param values - Array of 5-bit values
 ...
 * @param hrp - Human-readable part (lowercase)
 * @param data - Data including checksum (5-bit values)
 ...
 * @param hrp - Human-readable part (lowercase)
 * @param data - Data values (5-bit)
 * @param encoding - "Bech32" or "Bech32m"
 * @returns 6 checksum values
 */
 ...
 * @returns 5-bit words
 */
export function toWords(data: number[] | Uint8Array): number[] {
  let value = 0;
  let bits = 0;
  const result: number[] = [];
 ...
 * @param encoding - "Bech32" or "Bech32m"
 * @param segwit - If true, treat first byte as witness version (for Bitcoin SegWit)
...
export function encode(
  ...
  for (let i = 0; i < hrp.length; i++) {
    ...
    if (c < 33 || c > 126) {
      throw new OperationError(
        `HRP contains invalid character at position ${i}. Only printable ASCII characters (33-126) are allowed.`,
      );
    }
  }
  ...
  if (segwit && data.length >= 2) {
    ...
    const witnessProgram = Array.prototype.slice.call(data, 1) as number[];
 ...
 * @param encoding - "Bech32", "Bech32m", or "Auto-detect"
 * @returns Decoded HRP and data bytes
 */
export function decode(
  ...
  for (let i = 0; i < hrp.length; i++) {
    ...
    if (c < 33 || c > 126) {
      throw new OperationError(
        `HRP contains invalid character at position ${i}.`,
      );
    }
  }
  ...
  for (let i = 0; i < dataPart.length; i++) {
    ...
    if (CHARSET_REV[c] === undefined) {
      throw new OperationError(
        `Invalid character '${c}' at position ${sepIndex + 1 + i}.`,
      );
    }
  ...
  if (encoding === "Bech32") {
    ...
    if (verifyChecksum(hrp, data, "Bech32")) {
      ...
      throw new OperationError(
        "Invalid Bech32/Bech32m string: checksum verification failed.",
      );
    }
  }
  ...
  const couldBeSegWit =
    segwitHrps.includes(hrp) && words.length > 0 && words[0] <= 16;

  let bytes: number[];
  let witnessVersion: number | null = null;
  ...
  if (couldBeSegWit) {
    ...
    try {
      ...
      const validV0 =
        witnessVersion === 0 &&
        (programBytes.length === 20 || programBytes.length === 32);
      const validOther =
        witnessVersion !== 0 &&
        programBytes.length >= 2 &&
        programBytes.length <= 40;
      ...
      try {
        ...
        throw new OperationError(
          `Failed to decode data: ${(e2 as Error).message}`,
        );
      }
    }
  } else {
    ...
    try {
      ...
      throw new OperationError(
        `Failed to decode data: ${(e as Error).message}`,
      );
    }
  }
