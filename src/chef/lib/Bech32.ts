 ...
 */
function verifyChecksum(hrp: string, data: number[], encoding: string): boolean {
 ...
 */
function createChecksum(hrp: string, data: number[], encoding: string): number[] {
    const constant = encoding === "Bech32m" ? BECH32M_CONST : BECH32_CONST;
 ...
 */
export function encode(hrp: string, data: number[] | Uint8Array, encoding = "Bech32", segwit = false): string {
    // Validate HRP
    if (!hrp || hrp.length === 0) {
 ...
 */
export function decode(str: string, encoding = "Auto-detect"): Bech32DecodeResult {
    ...
    if (couldBeSegWit) {
        ...
        try {
            ...
            try {
                ...
                throw new OperationError(`Failed to decode data: ${(e2 as Error).message}`);
            }
        }
    } else {
        ...
        try {
            ...
            throw new OperationError(`Failed to decode data: ${(e as Error).message}`);
        }
    }
