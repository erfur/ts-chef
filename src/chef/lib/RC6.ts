 ...
 * @returns P constant for word size w
 */
function getP(w: number): bigint {
 ...
 * @param w - Word size in bits
 * @returns Q constant for word size w
 */
function getQ(w: number): bigint {
 ...
 */
export function getBlockSize(w: number): number {
    return 4 * (w / 8);
}
 ...
 * @param w - Word size in bits
 * @returns Recommended rounds
 */
export function getDefaultRounds(w: number): number {
    if (w <= 16) return 16;
    if (w <= 32) return 20;
    if (w <= 64) return 24;
    return 28;
 ...
 * @param w - Word size in bits
 * @returns Mask with w bits set
 */
function wordMask(w: number): bigint {
 ...
 * @param w - Word size in bits
 * @param lgMask - Mask for lower lg(w) bits
 * @returns Rotated value
 */
function ROL(x: bigint, n: bigint, w: number, lgMask: bigint): bigint {
 ...
 * @returns Rotated value
 */
function ROR(x: bigint, n: bigint, w: number, lgMask: bigint): bigint {
    const mask = wordMask(w);
    // Mask to lg(w) bits, then mod w for non-power-of-2 word sizes
    // For power-of-2, (n & lgMask) < w always, so mod w is no-op
    const shift = (n & lgMask) % BigInt(w);
 ...
 * @param w - Word size in bits
 * @returns Array of w-bit words as BigInt
 */
function bytesToWords(bytes: number[], w: number): bigint[] {
 ...
 */
function wordsToBytes(words: bigint[], w: number): number[] {
    const bytesPerWord = w / 8;
    const bytes: number[] = [];
 ...
 *
 * @param key - User key as byte array
 * @param rounds - Number of rounds
 * @param w - Word size in bits
 * @returns Array of 2r+4 subkeys as BigInt
 */
function generateSubkeys(key: number[], rounds: number, w: number): bigint[] {
    const bytesPerWord = w / 8;
    const b = key.length;
 ...
 *
 * @param block - Plaintext block (4*w/8 bytes)
 * @param S - Subkeys array
 * @param rounds - Number of rounds
 * @param w - Word size in bits
 * @returns Ciphertext block
 */
function encryptBlock(block: number[], S: bigint[], rounds: number, w: number): number[] {
    const mask = wordMask(w);
 ...
 *
 * @param block - Ciphertext block (4*w/8 bytes)
 * @param S - Subkeys array
 * @param rounds - Number of rounds
 * @param w - Word size in bits
 * @returns Plaintext block
 */
function decryptBlock(block: number[], S: bigint[], rounds: number, w: number): number[] {
    const mask = wordMask(w);
    const lgw = BigInt(Math.floor(Math.log2(w)));
    const lgMask = (1n << lgw) - 1n;
 ...
 * @param padding - Padding type ("NO", "PKCS5", "ZERO", "RANDOM", "BIT")
 * @param blockSize - Block size in bytes
 * @returns Padded message
 */
function applyPadding(message: number[], padding: string, blockSize: number): number[] {
 ...
 */
function removePadding(message: number[], padding: string, blockSize: number): number[] {
